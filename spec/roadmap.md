...
- [x] 定时策略：夜间窗口自动调暗/关闭 LED，白天自动恢复（`/api/night-schedule`）
- [x] 故障告警：磁盘消失 / 网络断开 debounce 后红色闪烁告警 + UI 横幅（`/api/alerts`）
- [x] 首次登录强制修改默认密码（阻断式弹窗）
- [x] fnOS 桌面 iframe 嵌入式窗口（`type: iframe` + CSP frame-ancestors）
- [x] 版本号运行时/包/文档对齐

## 未来计划

- [ ] **fnOS 通知中心**：异常时推送系统通知（官方暂无第三方推送 API，已用应用内告警替代）
- [ ] **快捷面板**：桌面小组件显示 LED 状态
- [ ] **多语言**：英文 UI
